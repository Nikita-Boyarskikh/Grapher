...
class Edge:
    def __init__(self, distance_to_farthest_point = 0, 
        ...
        self.distance_to_farthest_point = distance_to_farthest_point
        self.first_edge_shift_from_first_point = first_edge_shift_from_first_point
        self.second_edge_shift_from_first_point = second_edge_shift_from_first_point
    ...
    def find_abs_center(self, limit=0):
        ...
        if (limit <= 0):
            ...
            if min_node < min_edge[0]:
                ...
                return [Edge(x[0], x[1], x[1], x[2], x[3], x[4]) for i, x in enumerate(edges) if math.fabs(x[0]-min_edge[0]) < eps]
        else: 
            best_segments = self.find_best_segments(limit)
            if best_segments:
                res = [Edge(limit, x[3], x[4], x[0], x[1], x[2]) for x in best_segments]
                return res
        return None
